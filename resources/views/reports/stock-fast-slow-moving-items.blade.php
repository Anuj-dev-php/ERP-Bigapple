...
@section('content')
<h4 class="menu-title  mb-5 font-size-18 addeditformheading">Stock @if($mode=="fast") Fast @else Slow @endif Moving Items</h4>
...
<form method='post' @if($mode=="fast")  action="{{url('/')}}/{{$companyname}}/fast-moving-items" @else  action="{{url('/')}}/{{$companyname}}/slow-moving-items" @endif   >
	<input type="hidden"  name="mode"  value="{{$mode}}"  id="report_mode" />
  @csrf
		<div class='row'>
		<!-- -->
				...
				<!-- <div class="form-group col-5   mtb-1">
					...
					<div class="form-group col-6   mtb-1">
						...
						<select class='form-control' name="location">
							...
							<option value="{{$location_key}}" @if(  $location==$location_key) selected @endif>{{$location_name}}</option>
                          @endforeach
						</select> 
						   ...
						   <div class="form-group col-6    mtb-1">
							<label class="lbl_control_inline">Qty :</label>
							<div class="inline_control">
						...
						<div class="form-group col-6    mtb-1">
							<label class="lbl_control_inline">Qty Period:</label>
							<div class="inline_control">
						 		<select class='form-control' name="qty_period">
									<option value="day"  @if($qty_period=='day') selected @endif>Day</option>
									<option value="week"  @if($qty_period=='week') selected @endif>Week</option>
									<option value="month" @if($qty_period=='month') selected @endif>Month</option>
									<option value="qtr" @if($qty_period=='qtr') selected @endif>Qtr</option>
									<option value="year"  @if($qty_period=='year') selected @endif>Year</option>
								</select>
							</div>
					...
					<div class="form-group col-12 mtb-1 " style='text-align:center;'>
						...
						<a class="btn btn-primary" href="{{url('/')}}/{{$companyname}}/reset-stock-movement/{{$mode}}">Cancel</a>
						</div>
				</div>
			</div>
		</div>
...
@include('reports.send_report_modal')
		<div class='row'>
			...
			<input type="button" class="btn btn-primary"  value="XLSX" onclick="downloadDocument('xlsx')" />
						...
						&nbsp; 		&nbsp; 	<input type="button" class="btn btn-primary btnsendreport" data-mode="email" data-generateurl="{{route('company.send-stock-fast-slow-moving-items' ,['company_name'=>$companyname,'mode'=>$mode])}}"     value="Email" />

						
						&nbsp; 		&nbsp; 	<input type="button" class="btn btn-primary btnsendreport" data-mode="whatsapp"   data-generateurl="{{route('company.send-stock-fast-slow-moving-items' ,['company_name'=>$companyname,'mode'=>$mode])}}"     value="Whatsapp"   />
		 		@endif
      <div class="card">
					<div class="card-body">
						<div class=" mx-auto table-responsive"  style="max-height:400px;overflow:scroll;" >
                         <table class="table  table-bordered taboncell" >
							...
							<th style='min-width:100px;'>Product Id</th>
								...
								<th style='min-width:100px;'>Closing Qty</th>
								@if($show_amount==true)
								<th style='min-width:150px;'>Closing Amount</th>
								@endif
								<th style='min-width:200px;'>Avg Qty For Period</th>
								<th style='min-width:150px;'>Total Out</th>
								@foreach ($months as $month_key=>$month_name)
								<th  >{{$month_name}} Out</th> 				
									@endforeach
							...
							<td style="min-width:150px;"  tabIndex="1">{{  $closing_balance}}</td> 
							@if($show_amount==true)
							
							<td style="min-width:150px;"  tabIndex="1">{{$closing_amount}}</td> 

							@endif

							<td style="min-width:150px;"  tabIndex="1">{{$avg_qty_for_period}}</td> 
							
							<td style="min-width:150px;"  tabIndex="1">{{$total_month}}</td> 
 
							@foreach ($months as $month_key=>$month_name)
								<td style='min-width:150px;'   tabIndex="1">{{	$month_data[$month_key]}}</td> 				
									@endforeach 
                              </tr>
							@endforeach
...
function downloadDocument(format){
   ...
   var url="{{url('/')}}/{{$companyname}}/download-slow-fast-moving-items/"+mode+"/"+format;
  window.open(url);
