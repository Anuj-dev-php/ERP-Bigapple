...
@section('content')
<h4 class="menu-title  mb-5 font-size-18 addeditformheading">Stock Statement</h4>
...
<form method='post' action="{{url('/')}}/{{$companyname}}/stock-statement"   >
  @csrf
		<div class='row'>
		<!-- -->
			...
			<div class="col-md-8"   >
				...
				<!-- <div class="form-group col-5   mtb-1">
					...
					<div class="form-group col-12 mtb-1 " style='text-align:center;'>
						...
						<a class="btn btn-primary" href="{{url('/')}}/{{$companyname}}/reset-stock-statement">Cancel</a>
						</div>
				</div>
			</div>
		</div>
                         ...
                         <table class="table  table-bordered taboncell" >
							...
							<th style='min-width:100px;'>Product Id</th>
								...
								@if ($selected_location_id!="all")
								 <th  > {{$locations[$selected_location_id]}}  Balance</th>	
								 @if($show_amount==true)	 
								 <th   style='min-width:150px;'>{{$locations[$selected_location_id]}}  Amount</th>	
								 @endif
								@else
							...
							@endphp
							<tr>
							<td style="min-width:100px;">{{$selected_product}}</td> 
							<td style="min-width:150px;">{{   $product_name}}</td> 

							<td style="min-width:150px;">{{   $parent_name}}</td> 
							
							<td style="min-width:150px;">{{   $product_type}}</td> 
								...
								@foreach ($selected_locations as $selected_location_id  )
									...
									@if ($selected_location_id!="all")
									 ...
									 @if($show_amount==true)	
									 		<td   style='min-width:150px;'>{{$statement_data[$selected_location_id]['amount']}}</td>	
											@endif
									@else
...
var url="{{url('/')}}/{{$companyname}}/download-stock-statement/"+format;
  window.open(url);
