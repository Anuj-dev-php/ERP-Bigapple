...
@section('content')
<h4 class="menu-title  mb-5 font-size-18 addeditformheading">Reorder Report</h4>
...
<form method='post' action="{{url('/')}}/{{$companyname}}/reorder-report"   >
  @csrf
		<div class='row'>
		<!-- -->
			...
			<div class="col-md-8"   >
				...
				<!-- <div class="form-group col-5   mtb-1">
					...
					<div class="form-group col-12 mtb-1 " style='text-align:center;'>
						...
						<a class="btn btn-primary" href="{{url('/')}}/{{$companyname}}/reset-reorder-report">Cancel</a>
						</div>
				</div>
			</div>
		</div>
...
@include('reports.send_report_modal')
		<div class='row'>
			...
			<input type="button" class="btn btn-primary"  value="XLSX" onclick="downloadDocument('xlsx')" />
						...
						&nbsp; 		&nbsp; 	<input type="button" class="btn btn-primary btnsendreport" data-mode="email" data-generateurl="{{route('company.send-stock-reorder-report' ,['company_name'=>$companyname])}}"     value="Email" />

						
						&nbsp; 		&nbsp; 	<input type="button" class="btn btn-primary btnsendreport" data-mode="whatsapp"   data-generateurl="{{route('company.send-stock-reorder-report' ,['company_name'=>$companyname])}}"     value="Whatsapp"   />
		 		@endif
      <div class="card">
					<div class="card-body">
                         ...
                         <table class="table  table-bordered taboncell" >
							...
							<th style='min-width:100px;'>Product Id</th>
								...
								@if($show_amount==true)	
								 ...
								 <th style="min-width:150px;" > {{$locations[$selected_location_id]}} Cl. Bal Qty</th>	
									...
									<th style="min-width:150px;" >{{$locations[$selected_location_id]}} Reorder Qty</th>	
								
								<th style="min-width:150px;" >{{$locations[$selected_location_id]}} Difference</th>	
								@else
								@if($show_amount==true)	
								...
								<th style="min-width:150px;" >Total Reorder Qty</th>	
								
								<th style="min-width:150px;" >Total Difference</th>	
							...
							<td style="min-width:150px;"  tabIndex="1">{{   $product_type}}</td> 
								...
								@if($show_amount==true)	
									...
									<td   tabIndex="1">{{  $reorder_data[$selected_location_id]['closing_amount']}}</td>
									@endif	
                                  
                                    <td   tabIndex="1" > {{  $reorder_data[$selected_location_id]['closing_balance']}}</td>		
							
									<td   tabIndex="1">{{  $reorder_data[$selected_location_id]['reorder_qty']}}</td>
								<td   tabIndex="1"  @if($reorder_data[$selected_location_id]['diff']<0) style="background-color:yellow;" @endif>{{  $reorder_data[$selected_location_id]['diff']}}</td>
                                @else	
								@if($show_amount==true)		
												<td   tabIndex="1">{{ $reorder_data['all']['closing_amount']}}</td>
												@endif	
 
                                                                
											<td   tabIndex="1" > {{ $reorder_data['all']['closing_balance']}}</td>	
										
												<td   tabIndex="1">{{  $reorder_data['all']['reorder_qty']}}</td>
								<td   tabIndex="1"   @if($reorder_data['all']['diff']<0) style="background-color:yellow;" @endif>{{  $reorder_data['all']['diff']}}</td>
...
var url="{{url('/')}}/{{$companyname}}/download-reorder-report/"+format;
  window.open(url);
